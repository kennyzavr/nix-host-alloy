...
class IndexStateCorruptedError(AlloyError):
    def __init__(
        self, name: str, fact_name: str, reason: str = "cannot be parsed as valid JSON"
    ):
        self.name = name
        self.fact_name = fact_name
        self.reason = reason
        self.error_msg = (
            f"State for index '{name}' (fact '{fact_name}') is corrupted: {reason}."
        )
        super().__init__(self.error_msg)
...
class MasterSecretFileAlreadyExistsError(AlloyError):
    def __init__(self, master_secret_name: str, master_secret_file: Path):
        ...
        super().__init__(
            f"master_secret={master_secret_name} file={master_secret_file}"
        )
...
class HostSecretFileAlreadyExistsError(AlloyError):
    def __init__(self, host_secret_name: str, host_secret_file: Path, host_name: str):
        ...
        super().__init__(
            f"host={host_name} host_secret={host_secret_name} file={host_secret_file}"
        )
...
class JailSecretFileAlreadyExistsError(AlloyError):
    def __init__(self, jail_secret_name: str, jail_secret_file: Path, jail_name: str):
        ...
        super().__init__(
            f"jail={jail_name} jail_secret={jail_secret_name} file={jail_secret_file}"
        )
...
class QemuNotDefinedError(AlloyError):
    def __init__(self, vm_name: str):
        self.vm_name = vm_name
        super().__init__(f"vm={vm_name}")


class QemuNoMatchError(AlloyError):
    def __init__(self):
        super().__init__("No VMs match the given criteria.")


class QemuVariantNotFoundError(AlloyError):
    def __init__(self, vm_name: str, variant_name: str, available: list[str]):
        self.vm_name = vm_name
        self.variant_name = variant_name
        self.available = available
        super().__init__(
            f"vm={vm_name} variant={variant_name} available={','.join(available)}"
        )


class QemuProcessError(AlloyError):
    def __init__(self, vm_name: str, return_code: int):
        self.vm_name = vm_name
        self.return_code = return_code
        super().__init__(f"vm={vm_name} exit_code={return_code}")


class QemuAlreadyRunningError(AlloyError):
    def __init__(self, vm_name: str, pid: int):
        self.vm_name = vm_name
        self.pid = pid
        super().__init__(f"vm={vm_name} pid={pid} error=VM is already running")
