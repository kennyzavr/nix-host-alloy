  ...
  flake.alloyModules.services =
    ...
    let
      ...
      mkService =
        srvName: srv:
        let
          gateway = alloy.gateways.${srvName};
          routes = gateway.dns.routes;

          allOverlays = lib.pipe routes [
            (lib.mapAttrsToList (
              _: route: lib.map (target: target.overlay) alloy.endpoints.${route.endpoint}.targets
            ))
            lib.flatten
            lib.unique
          ];
        ...
        in
        {
          assertions = [
            {
              ...
              message = "[Alloy] dns-gateway '${srvName}': there are some dns endpoint targets with addresses outside of the allowed overlays";
            }
            {
              assertion = srv.hosts != { };
              message = "[Alloy] dns-gateway '${srvName}': at least one host must be specified";
            }
          ]
          ++ (lib.flatten (
            lib.mapAttrsToList (hostName: hostCfg: [
              {
                ...
                message = "[Alloy] dns-gateway '${srvName}': host '${hostName}' is unknown";
              }
              {
                assertion = builtins.hasAttr hostName alloy.hosts -> (hostCfg.ipv4 != null || hostCfg.ipv6 != null);
                message = "[Alloy] dns-gateway '${srvName}': host '${hostName}' must have specified at least one ip address (ipv4 or ipv6)";
              }
            ]) srv.hosts
          ));

          gateways.${srvName}.dns.entrypoints = lib.mapAttrs (_: hostCfg: {
          ...
          jails = lib.mapAttrs' (
            hostName: hostCfg:
            lib.nameValuePair "dns-gateway-${hostName}" (
              { config, ... }:
              let
                jail = config;
              in
                ...
                nixosModule = { pkgs, ... }: {
                  ...
                  services.dnsdist = {
                    ...
                    extraConfig = ''
                      ...
                      ${lib.concatMapAttrsStringSep "\n" (routeName: route: ''
                        ${lib.concatImapStringsSep "\n" (
                          ...
                          lib.optionalString (!target.down) ''
                            newServer({
                              address = "[${target.ipv6}]:${toString target.port}",
                              pool = "${routeName}",
                          ...
                          ''
                        ) alloy.endpoints.${route.endpoint}.targets}

                        setPoolServerPolicy(${
                          let
                            endpoint = alloy.endpoints.${route.endpoint};
                          in
                      ...
                      ${lib.concatMapStringsSep "\n" ({ suffix, routeName, ... }: ''
                        addAction("${suffix}", PoolAction("${routeName}"))
                      '') suffixes}
                    '';
                  };
                };
    ...
    in
    {
      options.services.dns-gateways = lib.mkOption {
        default = { };
      ...
      config =
        let
          services = lib.pipe alloy.services.dns-gateways [
