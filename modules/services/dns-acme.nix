    ...
    let
      ...
      mkService =
        ...
        in
        {
          ...
          endpoints.${srv.endpoint}.targets = lib.mapAttrsToList (overlayName: overlay: {
            ipv6 = alloy.jails.${jailName}.overlays.${overlayName}.ipv6;
            overlay = overlayName;
            port = 53;
          }) srv.overlays;

          gateways.${srv.gateway}.dns.routes."dns-acme-${srvName}" = {
            suffixes = lib.map (p: "${lib.removeSuffix "." srv.subzone}.${getParentApex p}") uniqueParentZones;
            endpoint = srv.endpoint;
          };

          dns.zones = newZones;
          dns.records = cnameRecords ++ parentDelegationRecords ++ subzoneRecords;
          ...
          config.package =
            ...
            pkgs.writeShellApplication {
              ...
              text = ''
                ...
                "$ALLOY_BIN" secrets set "${config.secret}" <<< "$tsig_key"
              '';
            };
        };
      };
